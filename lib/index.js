...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  mkdirp.sync(path.resolve(self.options.extendedModelsDirectory));
  ...
  if (self.options.extendedModelsDirectory !== null) {
    async.each(tables, createExtendedFile, !self.options.eslint ? callback : function() {
      var engine = new CLIEngine({ fix: true });
      var report = engine.executeOnFiles([self.options.extendedModelsDirectory]);
      CLIEngine.outputFixes(report);
      callback();
    });
  }
  ...
  function createFile(table, _callback) {
    var fileName = self.options.camelCaseForFileName ? _.camelCase(table) : table.charAt(0).toUpperCase() + table.slice(1);
    fs.writeFile(path.resolve(path.join(self.options.directory, fileName + '.autogenerated' + (self.options.typescript ? '.ts' : '.js'))), attributes[table], _callback);
  }

  function createExtendedFile(table, _callback) {
    var fileName = self.options.camelCaseForFileName ? _.camelCase(table) : table.charAt(0).toUpperCase() + table.slice(1);
    var pathNewFile = path.resolve(path.join(self.options.extendedModelsDirectory, fileName + (self.options.typescript ? '.ts' : '.js')));
    var replativePathAutogenerated = path.relative(self.options.extendedModelsDirectory, self.options.directory);

    var replaceOptions = {
      files: pathNewFile,
      from: [/{{ModelName}}/g, /{{AutogeneratedPath}}/g],
      to: [fileName, './' + path.join(replativePathAutogenerated, fileName + '.autogenerated' + (self.options.typescript ? '.ts' : '.js'))]
    };

    fs.stat(pathNewFile, function(err, stat) {
      if (err !== null && err.code == 'ENOENT') {
        fs.readFile(path.join(__dirname, '/utils/sample.model.extended'), 'utf8', function (err, data) {
          if (err) throw err;
          //Do your processing, MD5, send a satellite to the moon, etc.
          fs.writeFile (pathNewFile, data, function(err) {
            if (err) throw err;
